...
When an image file is uploaded into the `uploads/` folder in an S3 bucket (simulated via LocalStack), an **S3 ObjectCreated event** automatically triggers a **Lambda function**. The Lambda performs **mock image analysis (simulated AI label detection)**, stores the result as a JSON file in the `results/` folder of the same bucket, and saves metadata into a **DynamoDB table**.
...
4. Lambda stores output JSON in **S3 `results/`**
